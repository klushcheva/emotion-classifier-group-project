...
Набор данных представляет собой коллекцию постов в социальных сетях, относящихся к одной из республик РФ, за ноябрь 2023 года. В общей сложности в нем содержится 354 117 записей, которые отражают широкий спектр пользовательского контента на vk.com. Вот разбивка содержимого набора данных:

```id``` - идентификатор сообщения, облегчающий поиск и управление данными;

```text``` - фактическое содержание поста, дающее представление об общественном мнении, интересах и региональном дискурсе;

```created_at``` - временная метка, указывающая на время публикации поста, полезная для анализа тенденций и временных характеристик;

```sentiment``` - эмоциональная окраска (позитивная, негативная, нейтральная) дает представление об эмоциональном тоне дискуссии;

```emotion``` - в этом поле указываются конкретные эмоции, переданные в сообщении, такие как страх, удивление, радость, печаль, гнев или отсутствие каких-либо заметных эмоций. Это может быть полезно для понимания реакции общества на события или темы;

```toxicity``` - булевый индикатор, различающий грубые (токсичные) и негрубые сообщения, что важно для модерации контента и оценки здоровья сообщества;

```is_congratilate``` - еще один булев флаг, указывающий, содержит ли сообщение поздравления с каким-либо событием, выделяя сообщения праздничного характера;

```Spam``` - определяет, считается ли сообщение спамом, помогая отфильтровать нерелевантный или нежелательный контент.
...
Дальнейшие шаги к созданию сервиса с моделью:

1. Создание веб-приложения, которое позволит пользователю взаимодействовать с моделью (Streamlit)
2. Создание телеграм бота и тестов к нему (Pytest)
