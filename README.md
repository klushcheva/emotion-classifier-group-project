...
```sentiment``` - эмоциональная окраска (позитивная, негативная, нейтральная)
```emotion``` - эмоция (страх, удивление, радость, печаль, гнев, отсуствие эмоции)
```toxicity``` - булевый признак, грубое или не грубое сообщение
```is_congratilate``` - булевый признак, есть ли в сообщении поздаврление с событием
```Spam``` - булевый признак, являетяс ли сообщение спамом.
